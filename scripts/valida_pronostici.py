...
Valida la coerenza dei tabelloni inseriti dai giocatori nel file pronostici.json.
Rileva errori di logica come:
- Squadre eliminate inserite nei turni successivi.
- Scontri diretti in cui vengono qualificate entrambe le squadre o nessuna squadra.
- Numero errato di squadre selezionate per ciascuna fase.
"""
...
def main():
    print("=" * 60)
    print("VALIDAZIONE COERENZA TABELLONI GIOCATORI")
    print("=" * 60)
    ...
    for giocatore, dati in pronostici.get("partecipanti", {}).items():
        passaggio = dati.get("passaggio_turno", {})
        if not passaggio or not any(passaggio.values()):
            continue
        ...
        for mid in range(89, 97):
            ...
            if not home_parent or not away_parent:
                # Errore già segnalato al livello precedente, saltiamo
                continue
        ...
        if errori:
            ...
            print(f"\n⚠️  Giocatore: {giocatore} ({len(errori)} errori di coerenza)")
            for err in errori:
                print(f"  - {err}")

    if count_totale_errori == 0:
        print("\n✅ Complimenti! Tutti i tabelloni dei giocatori sono coerenti al 100%.")
    else:
        print(f"\n❌ Trovati in totale {count_totale_errori} errori di coerenza nei tabelloni.")
    print("=" * 60)
